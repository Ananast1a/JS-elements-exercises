...
function cards() {
    ...
    .then(data => {
        data.forEach(({img, alimg, title, descr, price})  => {
...
function slider({container, slide, nextArrow, prevArrow, totalCounter, currentCounter, wrapper, field}) {
    ...
    // Навигация слайдера - создание точек
...
function timer(id, deadline) {
    ...
    function getTimeRemaining(endtime) {
        const t = Date.parse(endtime) - Date.parse(new Date()),
            days = Math.floor(t / (1000 * 60 * 60 * 24)), // Mathfloor округление до ближайшего целого; 
            hours = Math.floor((t / (1000 * 60 * 60) % 24)), // % делит наш первый аргумент на 24 и возвращает остаток
